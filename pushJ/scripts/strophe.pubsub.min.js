Strophe.addConnectionPlugin("pubsub",{_connection:null,init:function(conn)
	{
		this._connection=conn;Strophe.addNamespace("PUBSUB","http://jabber.org/protocol/pubsub");
...
},createNode:function(jid,service,node,options,call_back)
{
	...
	var form_field=Strophe.xmlElement("field",[["var","FORM_TYPE"],["type","hidden"]]);
	...
	for(var i in options){
		...
		c_options.appendChild(x)}iq.c("pubsub",{xmlns:Strophe.NS.PUBSUB}).c("create",{node:node}).up().cnode(c_options);
		this._connection.addHandler(call_back,null,"iq",null,iqid,null);
		this._connection.send(iq.tree());
		return iqid},subscribe:function(jid,service,node,options,event_cb,call_back)
		{
			...
			if(options&&options.length&&options.length!==0)
				{
					for(var i=0;i<options.length;i++){
						var val=options[i];x.appendChild(val)
					}
					sub_options.appendChild(x);
					sub.c("pubsub",{xmlns:Strophe.NS.PUBSUB}).c("subscribe",{node:node,jid:jid}).up().cnode(sub_options)
				}else{
					sub.c("pubsub",{xmlns:Strophe.NS.PUBSUB}).c("subscribe",{node:node,jid:jid})
				}
				this._connection.addHandler(call_back,null,"iq",null,subid,null);
				this._connection.addHandler(event_cb,null,"message",null,null,null);
				this._connection.send(sub.tree());
				return subid},unsubscribe:function(jid,service,node,call_back){
					var subid=this._connection.getUniqueId("unsubscribenode");
					var sub=$iq({from:jid,to:service,type:"set",id:subid});
					sub.c("pubsub",{xmlns:Strophe.NS.PUBSUB}).c("unsubscribe",{node:node,jid:jid});
					this._connection.addHandler(call_back,null,"iq",null,subid,null);
					this._connection.send(sub.tree());return subid},publish:function(jid,service,node,items,call_back){
						var pubid=this._connection.getUniqueId("publishnode");
						var publish_elem=Strophe.xmlElement("publish",[["node",node],["jid",jid]]);for(var i in items)
						{
							var item=Strophe.xmlElement("item",[]);
							var entry=Strophe.xmlElement("entry",[]);
							var t=Strophe.xmlTextNode(items[i]);entry.appendChild(t);
							item.appendChild(entry);publish_elem.appendChild(item)
						}
						var pub=$iq({from:jid,to:service,type:"set",id:pubid});
						pub.c("pubsub",{xmlns:Strophe.NS.PUBSUB}).cnode(publish_elem);
						this._connection.addHandler(call_back,null,"iq",null,pubid,null);
						this._connection.send(pub.tree());
						return pubid},items:function(jid,service,node,ok_callback,error_back)
						{
							var pub=$iq({from:jid,to:service,type:"get"});
							pub.c("pubsub",{xmlns:Strophe.NS.PUBSUB}).c("items",{node:node});
